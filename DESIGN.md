...
[FastAPI 서버]  src/api_server.py
  ...
  - /api/rollback/preview  : 롤백 대상 사전 확인
        ...
        ├──► [llm_client.py]       OpenAI gpt-4.1-mini 호출
...
OpenAI gpt-4.1-mini에 전달되는 컨텍스트:
...
파일당 예상 비용: GPT-4.1-mini 기준 약 **$0.0028** (입력 ~3,800 토큰 + 출력 ~700 토큰)  
...
| AI 메타데이터 추출 | OpenAI gpt-4.1-mini | .env OPENAI_API_KEY |
| HTTP 통신 | requests | |
...
llm:
  ...
  openai:
    ...
    api_key: ""   # 또는 .env의 OPENAI_API_KEY
```
